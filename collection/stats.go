...
func summarise(cs []cards.Card, coll *collection) []setStats {
	...
	// Printings fold onto nameKey, so that every printing of a card counts
	// once and copies are summed over all of them: three of a card make a
	...
	for _, c := range cs {
		...
		s, ok := stats[k.set]
		if !ok {
			s = &setStats{setID: k.set, label: c.Set.Label}
			stats[k.set] = s
			order = append(order, k.set)
		}
